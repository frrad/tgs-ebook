...
do
    ...
    url="http://gravitytales.com/novel/the-good-student/tgs-chapter-$x"
    if [[ ! -e $filename ]]; then
...
do
    ...
    if [[ ! -e $output ]]; then
        ...
        grep -A 1 "chapterContent" $input | tail -n 1 > $TMP1
...
do
    ...
    if [[ ! -e $output ]]; then
        ...
        #chapter titles
        sed -i 's;<span style="font-size: 18px">\(CHAPTER [^<]*\)</span>;# \1;' $input 
...
if [[ ! -e cover/raw_cover.jpg ]]; then
    ...
    curl -o cover/raw_cover.jpg https://cdn.gravitytales.com/images/covers/the-good-student.jpg
fi
